...
const DialogflowServer = require('../lib/platforms/dialogflow');
...
module.exports = function(RED) {
  ...
  RED.redbot.platforms.dialogflow = DialogflowServer;

  var contextProviders = ContextProviders(RED);

  function DialogflowBotNode(n) {
    ...
    var dialogflowConfigs = RED.settings.functionGlobalContext.get('dialogflow') || {};
    ...
    if (!startNode) {
      ...
      console.log(warn('Dialogflow Bot ' + this.botname + ' will NOT be launched, environment is ' + environment));
      return;
    }
    // eslint-disable-next-line no-console
    console.log(green('Dialogflow Bot ' + this.botname + ' will be launched, environment is ' + environment));
    ...
    if (dialogflowConfigs[node.botname] != null) {
      var validation = validators.platform.dialogflow(dialogflowConfigs[node.botname]);
      if (validation != null) {
        /* eslint-disable no-console */
        console.log('');
        console.log(lcd.error('Found a Dialogflow configuration in settings.js "' + node.botname + '", but it\'s invalid.'));
        ...
        console.log('');
        console.log(lcd.grey('Found a valid Dialogflow configuration in settings.js: "' + node.botname + '":'));
        console.log(prettyjson.render(dialogflowConfigs[node.botname]));
        console.log('');
        /* eslint-enable no-console */
        botConfiguration = dialogflowConfigs[node.botname];
      }
    }
    ...
    if (node.chat == null) {
      ...
      try {
        ...
        node.chat = DialogflowServer.createServer({
        ...
        RED.nodes.eachNode(function(currentNode) {
          if (currentNode.type === 'chatbot-extend' && !_.isEmpty(currentNode.codeJs)
            && currentNode.platform === 'dialogflow') {
            try {
  ...
  registerType('chatbot-dialogflow-node', DialogflowBotNode, {
    credentials: {}
  });

  function DialogflowInNode(config) {
    ...
    if (this.config) {
      ...
      if (node.chat) {
        ...
        nodeGlobalKey = 'dialogflow_master_' + this.config.id.replace('.','_');
        var isMaster = false;
        ...
        node.warn('Missing or incomplete configuration in Dialogflow Receiver');
      }
    } else {
      node.warn('Missing configuration in Dialogflow Receiver');
    }
  ...
  registerType('chatbot-dialogflow-receive', DialogflowInNode);

  function DialogflowOutNode(config) {
    RED.nodes.createNode(this, config);
    var node = this;
    var global = this.context().global;
    var environment = global.environment === 'production' ? 'production' : 'development';
  ...
  registerType('chatbot-dialogflow-send', DialogflowOutNode);
